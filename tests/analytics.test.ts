...
test('model trains with time holdout and useful discrimination',()=>{const result=trainRetentionModel(demoRows(),assumptions);assert.ok(result.metric.holdoutRows>50);assert.ok(result.metric.auc!==null);assert.ok((result.metric.auc??0)>.55);assert.ok(result.trainDateMax<=result.holdoutDateMin);});
